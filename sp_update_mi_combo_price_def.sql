...
SET @entry = (SELECT TOP 1 combo_menu_item_seq
                FROM micros.mi_combo_price_def
            ORDER BY combo_menu_item_seq DESC);
...
tier: WHILE @i < @entry LOOP
    ...
    SET @nprice = (SELECT TOP 1 "preset_amt_1"
                     FROM micros.mi_combo_price_def
                    WHERE combo_menu_item_seq = @i
                      AND where price_tier_seq = 50);

    IF @nprice IS NOT NULL THEN
        UPDATE micros.mi_combo_price_def
